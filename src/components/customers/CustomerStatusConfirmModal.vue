...
interface Props {
  modelValue: boolean
  customer: CustomerAdminItem | null
  loading: boolean
}
...
<template>
  <Teleport to="body">
    <Transition name="modal">
      <div
        v-if="props.modelValue && props.customer"
      ...
      >
        <div class="w-full max-w-[420px] rounded-3xl bg-white p-6 shadow-lg">
          <div class="space-y-3">
            <p class="text-xs font-semibold uppercase tracking-[0.18em] text-[#F59E0B]">Cập nhật trạng thái khách hàng</p>
            <h3 class="text-xl font-bold text-[#4A2A12]">Xác nhận {{ getActionText(props.customer.accountStatus) }} tài khoản này?</h3>
            <p class="text-sm leading-6 text-gray-500">
              Khách hàng <strong>{{ props.customer.name }}</strong> ({{ props.customer.phone }})
              sẽ chuyển từ
              <strong>{{ props.customer.accountStatus === 'ACTIVE' ? 'Hoạt động' : 'Tạm khóa' }}</strong>
              sang
              <strong>{{ getNextStatus(props.customer.accountStatus) === 'ACTIVE' ? 'Hoạt động' : 'Tạm khóa' }}</strong>.
            </p>
          </div>
          ...
          <div class="mt-6 flex justify-end gap-3">
            ...
            <button
              ...
              @click="emit('submit', { userId: props.customer.id, accountStatus: getNextStatus(props.customer.accountStatus) })"
            >
